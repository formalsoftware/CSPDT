...
public class CspMHyperlinkDetector implements IHyperlinkDetector {
	...
	public CspMHyperlinkDetector(CspMEditor editor) {
		this.editor = editor;
	}
	...
	// PEGA O CONTEUDO DA REGION E COMPARA COM AS DEFINI��ES DO DOCUMENTO
	private IHyperlink createHyperLink(IDocument document, IRegion region, LinkedList<CspMRef> refs) {
		...
		if (key != null) {
			...
			for (CspMRef ref : refs) {
				if (key.equals(ref.getText())) {
					link = new CspMHyperlink(ref, this.editor, region);
					break;
				}
			}
		}
