...
public class CspMAstTreeNodeGenerator extends ExtendedDepthFirstAdapter {
	...
	private CspMOutlineTreeNode rootNode;

	private Stack<CspMOutlineTreeNode> parents;

	private boolean detailed;

	public CspMAstTreeNodeGenerator(IDocument document, boolean detailed) {
		this.document = document;
		this.detailed = detailed;
	}

	public CspMOutlineTreeNode getRootNode() {
		return this.rootNode;
	}

	public void inStart(Start node) {
		this.parents = new Stack<CspMOutlineTreeNode>();

		Position position = new Position(0, 0);
		try {
			this.document.addPosition(CspMOutlineContentProvider.SEGMENTS, position);
	...
	public void outStart(Start node) {
		CspMOutlineTreeNode rootNode = this.parents.pop();
...
//		parents.peek().add(thisNode);
		addToParent(new CspMOutlineTreeNode(node, computePosition(node)));
	}

	public void caseEOF(EOF node) {
	}

	private CspMOutlineTreeNode push(Node node, Position position) {
		CspMOutlineTreeNode treeNode = new CspMOutlineTreeNode(node, position);
		this.parents.push(treeNode);
		return treeNode;
	}

	private void pop() {
		CspMOutlineTreeNode treeNode = this.parents.pop();
		treeNode.updateChildren();
		addToParent(treeNode);
	}

	private void addToParent(CspMOutlineTreeNode treeNode) {
		CspMOutlineTreeNode parentTreeNode = this.parents.peek();
	...
	private void genDefinition(Node node, TName tName) {
		...
		addToParent(new CspMOutlineTreeNode(node, position));
	}
